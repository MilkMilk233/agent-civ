...
object AgentPromptBuilder {
    ...
    fun build(
        ...
        return """
            ...
            - Some surfaced unit options are short operational assignments rather than single-tile tactics. Options such as stage outside a border, reinforce an assault, assault a city ring, recover then rejoin, or preserve a capture unit are deliberate heuristics that are meant to be easier and safer than rebuilding the same march logic with raw unit_move every turn.
            ...
            - Read operational military unit options literally. stage outside border means prewar staging near the target. reinforce assault means march toward the active battlefield as the second line. assault city ring means move into a useful surround or firing slot around the target city. recover then rejoin means fall back to heal while staying on the objective axis. preserve capture unit means keep one healthy melee close enough to take the city later instead of trading it away early.
