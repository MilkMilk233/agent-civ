...
start_backend() {
  echo "Starting Unciv replay dashboard server on http://localhost:${UNCIV_AGENT_OBS_PORT:-7071}/ ..."
  cd "$ROOT_DIR"
  ...
  if [[ "${UNCIV_AGENT_USE_DIST_JAR:-false}" =~ ^(1|true|yes)$ ]] && [[ -f "$ROOT_DIR/desktop/build/libs/Unciv.jar" ]]; then
    ...
    exec java -jar "$ROOT_DIR/desktop/build/libs/Unciv.jar" --agenthistory
  fi
  exec ./gradlew desktop:run --args="--agenthistory"
}
